...
import data.scripts.weapons.OasOrbMissileEffect;
...
public class OasMissileOrbAi implements GuidedMissileAI, MissileAIPlugin {
	...
	private static final float RETARGET_ARC = 180.0f;
	private static final float SPLIT_DISTANCE = 400.0f;
	...
	private static final float TRIGGER_TIME = 1.05f;

	public OasMissileOrbAi(MissileAPI missile, ShipAPI launchingShip) {
	...
	public void advance(float amount) {
		...
		if (sinceLastDistance > 0.1f && !triggered) {
			...
			if (distance < SPLIT_DISTANCE) {
				...
				//if (isInArc || inRangeCount > 15) {
					triggered = true;
					Global.getSoundPlayer().playSound("oas_l_eorb_trigger", 1.0f, 0.65f, missile.getLocation(), missile.getVelocity());
				//}
				//inRangeCount++;
			}
		}
		if(triggered) {
			...
			float range = (1 - Math.min(1.0f, sinceTrigger / TRIGGER_TIME)) * 10.0f;
			missile.setJitter(missile, missile.getSpec().getExplosionColor(), intensity, 5, range);
			missile.giveCommand(ShipCommand.DECELERATE);
		} else {
			missile.giveCommand(ShipCommand.ACCELERATE);
		}
		if(triggered && sinceTrigger > TRIGGER_TIME && !splitted) {
			split();
		}
	}

	public float getTriggerLevel() {
		float level = Math.min(1.0f, sinceTrigger / TRIGGER_TIME);
		return level;
	}
	...
	private void split() {
        ...
        float dam = missile.getBaseDamageAmount();
        EmpArcEntityAPI arc = engine.spawnEmpArc(missile.getSource(), missile.getLocation(), missile,
						target,
						DamageType.ENERGY, 
						dam,
						emp, // emp 
		...
		missile.setFlightTime(0.0f);
		missile.flameOut();
		missile.setDamageAmount(0.0f);
		engine.applyDamage(missile, missile.getLocation(), 0.0f, DamageType.ENERGY, 100000.0f, true, false, null, false);
		splitted = true;
    }
	...
	private class TargetAnalysis {
		...
		public TargetAnalysis() {
			...
			final float randomOffsetX = randomOffsetDir.x * courseRandomLevel * randomTarget * maxRadius * 2.0f;
			final float randomOffsetY = randomOffsetDir.y * courseRandomLevel * randomTarget * maxRadius * 2.0f;
