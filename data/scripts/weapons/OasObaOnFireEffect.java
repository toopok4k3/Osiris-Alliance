...
public class OasObaOnFireEffect implements OnFireEffectPlugin {
			...
			if(ship != null) {
				...
				if(shipVelocity != null && projectileVelocity != null && weapon.getSpec() != null) {
					...
					if(flashGlowColor != null) {
						...
						engine.addNebulaSmokeParticle(location, shipVelocity, 80.0f, 0.1f, 1.0f, 1.0f, 0.2f, flashGlowColor);
						for(int i = 0; i < 20; i++) {
							final float scale = 0.2f*Misc.random.nextFloat();
							Vector2f velocity = new Vector2f(shipVelocity.x + projectileVelocity.x * scale, shipVelocity.y + projectileVelocity.y * scale);
							engine.addNebulaSmokeParticle(location, velocity, 5.0f, 1.5f + 8f * Misc.random.nextFloat(), 0.5f, 0.1f, 1.0f-scale, smokeColor);
						}
					}
				}
			}
